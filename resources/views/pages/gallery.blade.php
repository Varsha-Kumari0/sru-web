                        ...
                        <div class="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
                            <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-1">
                                @foreach($albums as $album)
                                    @php
                                        $coverImage = $album->cover_image ? asset('images/' . $album->cover_image) : asset('images/home-carosel1.jpeg');
                                    @endphp
                                    <article class="group w-full max-w-[170px] overflow-hidden rounded-sm bg-white text-center shadow-sm ring-1 ring-slate-100 transition hover:-translate-y-1 hover:shadow-md sm:max-w-none">
                                        ...
                                        <div class="px-3 py-3">
                                            <h2 class="text-sm font-semibold text-[#1d2f54]">{{ $album->title }}</h2>
                            ...
                            <div class="rounded-2xl border border-dashed border-slate-200 bg-gradient-to-br from-slate-50 to-white p-6">
                                <div class="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                                    <div>
                                        <p class="text-xs font-bold uppercase tracking-[0.28em] text-[#c0922d]">Featured collection</p>
                                        <h2 class="mt-3 text-2xl font-semibold text-[#1d2f54]">{{ $featuredAlbum?->title }}</h2>
                                        <p class="mt-3 max-w-2xl text-sm leading-7 text-slate-600">{{ $featuredAlbum?->summary ?: 'Explore recent alumni events and campus moments captured by the SRU community.' }}</p>
                                    </div>
                                    <div class="rounded-full bg-[#1d2f54] px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-white">
                                        {{ $featuredAlbum?->photo_count ?? 0 }} Photos
                                    </div>
                                </div>
                                ...
                                <div class="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                    @foreach($albums->take(3) as $album)
                                        <div class="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                                            <p class="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Album</p>
                                            <h3 class="mt-2 text-lg font-semibold text-[#1d2f54]">{{ $album->title }}</h3>
                                            <p class="mt-2 text-sm leading-6 text-slate-600">{{ $album->summary }}</p>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
