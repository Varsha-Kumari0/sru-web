...
<div class="min-h-screen bg-[#f4f6f9] py-8">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg shadow-sm border border-[#e2e8f0] p-6">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-[#1a2d5a]">Edit Skill</h1>
                <p class="text-sm text-gray-600 mt-1">Update your skill details.</p>
            </div>

            <form action="{{ route('skills.update', $skill) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    >
                        @foreach (['beginner', 'intermediate', 'advanced', 'expert'] as $level)
                            <option value="{{ $level }}" @selected(old('level', $skill->level) === $level)>
                                {{ ucfirst($level) }}
                            </option>
                        @endforeach
                    </select>
