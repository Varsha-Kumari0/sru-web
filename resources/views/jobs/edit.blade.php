    ...
    <div class="sru-hero-gradient relative overflow-hidden" style="height:150px;">
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-end pb-6">
            <div>
                ...
                <h1 class="text-3xl font-bold text-white tracking-tight">Edit Job or Internship</h1>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                <div>
                    ...
                    <p class="text-sm mt-1" style="color:#64748b;">Update the opening details for students and alumni.</p>
                </div>
            ...
            <form method="POST" action="{{ route('jobs.update', $job) }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input name="title" value="{{ old('title', $job->title) }}" required class="input" placeholder="Frontend Developer Intern">
                    </div>
                    <div>
                        <label class="label">Company Name <span class="text-red-500">*</span></label>
                        <input name="company_name" value="{{ old('company_name', $job->company_name) }}" required class="input" placeholder="Company or startup name">
                    </div>
                    <div>
                        <label class="label">Company Website</label>
                        <input type="url" name="company_website" value="{{ old('company_website', $job->company_website) }}" class="input" placeholder="https://company.com">
                    </div>
                    <div>
                        <label class="label">Contact Email <span class="text-red-500">*</span></label>
                        <input type="email" name="contact_email" value="{{ old('contact_email', $job->contact_email) }}" required class="input">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input name="location" id="locationInput" value="{{ old('location', $job->location) }}" class="input" placeholder="Hyderabad, Bengaluru, Remote">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input name="skills" value="{{ old('skills', implode(', ', $job->skills ?? [])) }}" required class="input" placeholder="Laravel, React, SEO">
                        <p class="text-xs mt-1" style="color:#64748b;">Separate skills with commas.</p>
                    </div>
                    <div>
                        <label class="label">Salary / Stipend</label>
                        <input name="salary" value="{{ old('salary', $job->salary) }}" class="input" placeholder="₹25,000/month or Negotiable">
                    </div>
                    <div>
                        <label class="label">Application Deadline</label>
                        <input type="date" name="application_deadline" value="{{ old('application_deadline', optional($job->application_deadline)->format('Y-m-d')) }}" class="input">
                    </div>
                    <div>
                        ...
                        @if($job->attachment)
                            <p class="text-xs mt-2" style="color:#64748b;">
                                Current: <a class="font-semibold hover:underline" style="color:#2a9d8f;" href="{{ asset('storage/' . $job->attachment) }}" target="_blank" rel="noopener noreferrer">
                                    {{ $job->attachment_original_name ?? 'Attachment' }}
                                </a>
                            </p>
                        @endif
                    </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="8" required class="input" placeholder="Describe responsibilities, eligibility, application steps, and anything candidates should know.">{{ old('description', $job->description) }}</textarea>
                </div>
